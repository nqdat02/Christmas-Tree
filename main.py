...
def make_ball(x, y, size, coulor, ball):
    ball.hideturtle()
    ball.penup()
...
screen.tracer(2)
ball_colours = ["red", "red", "red", "gold", "orange", "pink", "gold", "silver", "green", "white"]

ball_positions = [
    (-width/30, width/4), (3 * width / 40, width / 5),
...
for position in ball_positions:
    make_ball(position[0], position[1], 20 / i_scale, random.choice(ball_colours), turtle.Turtle())
    screen.update()
...
text_1.color("green")
text_1.write("Happy New Year!", font=("Apple Chancery", max(int(30/i_scale), 15), "bold"), align=("center"))

if web_base:text_1.setposition(width/2.95, -width / 2.14)
else: text_1.setposition(width/3.85, -width / 2.14)

if web_base:
    for _ in range(200):
        snow_fall()
else:
    while True: snow_fall()
turtle.done()